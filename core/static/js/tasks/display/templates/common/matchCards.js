/**
 * Рендерит задание на сопоставление карточек
 * @param {Object} task - Объект задания
 * @param {Object} task.data - Данные задания
 * @param {Array} task.data.shuffled_cards - Перемешанные пары карточек
 * @param {string} task.data.shuffled_cards[].card_left - Левая часть пары
 * @param {string} task.data.shuffled_cards[].card_right - Правая часть пары
...
export function renderMatchCardsTask(task, container) {
    ...
    if (!Array.isArray(task.data.shuffled_cards) || !task.data.shuffled_cards) {
        console.warn(
            "task.data.shuffled_cards не является массивом:",
            task.data.shuffled_cards
        );
